...
data_all_raw['Price'] = data_all_raw['Price']/1000000

# rename series for pairplot
data_all = data_all_raw.rename(columns = {'Home_size': 'Home size (sqft)', 
                                            'Lot_size': 'Lot size (sqft)',
                                            'Commute_time': 'Commute time (mins)',
...
# surroundings-only data
g = sns.pairplot(data_all, 
             plot_kws=dict(line_kws=dict(color = '#3148a3'), 
                           scatter_kws=dict(facecolor = '#5797ff', edgecolor = '#3148a3', 
                                            linewidth = 0.33, alpha = 0.1, s = 20)),
             x_vars = ['Commute time (mins)', 'School score (%)'], 
             y_vars = 'Price ($M)', kind = 'reg')
g.fig.text(0.15, 0.825, r'$R^{2}$ = ' + str(corr_commute_price), fontsize=10)
g.fig.text(0.60, 0.825, r'$R^{2}$ = ' + str(corr_school_price), fontsize=10)

g.savefig('pairplot_price_surroundings2.jpg', dpi=600)
